    ...
    <div class="max-w-7xl mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
            <div>
                ...
                <h1 class="text-2xl font-bold text-slate-800">Schools Details</h1>
            </div>
                ...
                <div class="overflow-x-auto w-full">
                    <table class="min-w-full divide-y divide-slate-200">
                        <thead class="bg-slate-50">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">School Name</th>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">School Email </th>
                        ...
                        <tbody class="bg-white divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50/50 transition">
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-900">{{ $d->school_name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{{ $d->school_email }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{{ $d->phone }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{{ $d->address }}</td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                                    <div class="flex items-center justify-center gap-2">
                                        ...
                                        <!-- View Details Action -->
                                        <form action="/school-profile" method="post" class="inline">
                                            @csrf
                                            <input type="hidden" value="{{ $d->school_email }}" name="email">
                                        ...
                                        <!-- Delete Action -->
                                        <form action="/school-delete" method="post" class="inline" onsubmit="return confirm('Are you sure you want to delete this School?')">
                                            @csrf
                                            <input type="hidden" value="{{ $d->school_email }}" name="email">
        ...
        @else
            ...
            <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-12 text-center max-w-md mx-auto mt-10">
                ...
                <h3 class="text-base font-semibold text-slate-800">No School Records</h3>
